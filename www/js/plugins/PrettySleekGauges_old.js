...
Window_MenuStatus.prototype.drawActorEXP = function(actor, x, y, width) {
	...
	this.drawExpGauge(x, y + 36, width, rate, this.textColor(0), this.textColor(8));

	this.changeTextColor(this.systemColor());
	this.drawText(TextManager.expA, x, y + 36, 44);
	this.changeTextColor(this.normalColor());
	this.drawText((Math.floor(rate * 10000) / 100) + "%", x, y + 36, width,"right");
};
...
Special_Gauge.prototype.initialize = function(x, y, w, r, c1, c2, basewindow, h, critText) {
	this._x = x;
	this._y = y;
	this._width = w;
...
Special_Gauge.prototype.update = function() {
	...
	if (!animatedNumbers) this._curVal = this._setVal;
	if (!animatedGauges) this._curRate = this._maxRate;
...
Window_EnemyHPBars.prototype.drawActorHp = function(actor, x, y, width) {
	...
	if (!$dataEnemies[actor.enemyId()].meta.HideEnemyHPBar) {
		barTypeLeft = $dataEnemies[actor.enemyId()].meta.BarTypeLeft || hpBarTypeLeft || barTypeLeft;
		barTypeRight = $dataEnemies[actor.enemyId()].meta.BarTypeRight || hpBarTypeRight || barTypeRight;

		this.drawAnimatedGauge(x, y, width, actor.hpRate(), this.hpGaugeColor1(), this.hpGaugeColor2());
	...
	if (shouldDrawEnemyMP && (drawEnemyMPWhenNoMP || actor.mmp > 0) && !$dataEnemies[actor.enemyId()].meta.HideEnemyMPBar) {
...
Window_EnemyHPBars.prototype.update = function() {
	...
	for (var i = 0; i < $gameTroop._enemies.length; i++) {
		...
		x = $gameTroop._enemies[i].screenX() - (width + this.textPadding())/2 + EHPXOffset + (parseInt($dataEnemies[$gameTroop._enemies[i].enemyId()].meta.HPBarXOffset) || 0);
		y = $gameTroop._enemies[i].screenY() - this.lineHeight() + EHPYOffset + (parseInt($dataEnemies[$gameTroop._enemies[i].enemyId()].meta.HPBarYOffset) || 0);
...
Sprite_Enemy.prototype.updateStateSprite = function() {
   ...
   this._stateIconSprite.y += (parseInt($dataEnemies[this._enemy.enemyId()].meta.HPBarYOffset) || 0);
   this._stateIconSprite.x = EHPStateXOffset + (parseInt($dataEnemies[this._enemy.enemyId()].meta.HPBarXOffset) || 0);
};
} // End Enemy HP bars
