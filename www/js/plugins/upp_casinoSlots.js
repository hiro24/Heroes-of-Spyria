 ...
 * @default 10
 *
 * @param Slot Win Sound
 * @desc Sound effect when selecting random slots.
 * @default Shop
 *
 * @param Slot Win Pitch
 * @desc Sound effect pitch when selecting random slots.
 * @default 100
 *
 * @param Slot Win Volume
 * @desc Sound effect volume when selecting random slots.
 * @default 70
 *
 * @param Ticks
...
(function() {
	...
	sounds[2] = {
		name: params['Slot Win Sound'],
		pitch: Number(params['Slot Win Pitch']), 
		volume: Number(params['Slot Win Volume']),
		pan: 0
	}
	...
	Fn("Window_SlotGraphics", "initialize", function() {
		Window_Base.prototype.initialize.call(this, 100, 0, Graphics.boxWidth, Graphics.boxHeight);
		this.opacity=0;
		this.slot_id = [];
		this.timer=0;
		this.ticks=0;
		...
		if(this.timer==speed && this.finished==false) {
			...
			if(this.ticks>=ticks*3+1) {
				...
				for(var i=0;i<victories.length;i++) {
					if(this.slot_id[0] == victories[i].s1 && victories[i].s2 == 8 && victories[i].s3 == 8) {
						$gameParty.gainGold(victories[i].win);
						/* upp_data().chips+=victories[i].win; */
						this.winAmount = victories[i].win;
						AudioManager.playSe(sounds[2]);
						this.win=true;
					} else if(this.slot_id[0] == victories[i].s1 && this.slot_id[1] == victories[i].s2 && victories[i].s3 == 8) {
						$gameParty.gainGold(victories[i].win);
						/* upp_data().chips+=victories[i].win; */
						this.winAmount = victories[i].win;
						AudioManager.playSe(sounds[2]);
						this.win=true;
					} else if(this.slot_id[0] == victories[i].s1 && this.slot_id[1] == victories[i].s2 && this.slot_id[2] == victories[i].s3) {
						$gameParty.gainGold(victories[i].win);
						/* upp_data().chips+=victories[i].win; */
						this.winAmount = victories[i].win;
						AudioManager.playSe(sounds[2]);
	...
	Chip_Window.prototype.refresh = function() {
		...
		this.contents.drawText($gameParty.gold(), 0, 0, this.contents.width, this .lineHeight(), 'right');
	}
	...
	Scene_UppSlots.prototype.yes = function() {
		...
		/* Modification to make the slot use real gold*/
		
		$gameParty.gainGold(-slotCost);
