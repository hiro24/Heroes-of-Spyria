...
Yanfly.Param.VASkipAfterFirstPlay = 0;
...
BattleManager.prepareVictoryInfo = function() {
	...
	this.gainRewards();
	// Added code for victory voice clips - CUSTOM 
    $gameParty.allMembers().forEach(function(actor) {
        actor._expGained = actor.currentExp() - actor._preVictoryExp;
        actor._postVictoryLv = actor._level;
		if (actor._actorId === 5) {
			Yanfly.Param.VAArray.push("Hiro_Victory_1");
			Yanfly.Param.VAArray.push("Hiro_Victory_2");
			Yanfly.Param.VAArray.push("Hiro_Victory_1");
		}
		if (actor._actorId === 6) {
			Yanfly.Param.VAArray.push("Madilyn_Victory_1");
			Yanfly.Param.VAArray.push("Madilyn_Victory_2");
			Yanfly.Param.VAArray.push("Madilyn_Victory_3");
		}
		if (actor._actorId === 7) {
			Yanfly.Param.VAArray.push("Winter_Victory_1");
			Yanfly.Param.VAArray.push("Winter_Victory_2");
			Yanfly.Param.VAArray.push("Winter_Victory_2");
		}
		if (actor._actorId === 8) {
			Yanfly.Param.VAArray.push("Davos_Victory_1");
			Yanfly.Param.VAArray.push("Davos_Victory_2");
			Yanfly.Param.VAArray.push("Davos_Victory_2");
		}
		if (actor._actorId === 9) {
			Yanfly.Param.VAArray.push("Auryn_Victory_1");
			Yanfly.Param.VAArray.push("Auryn_Victory_2");
			Yanfly.Param.VAArray.push("Auryn_Victory_2");
		}
		if (actor._actorId === 21) {
			Yanfly.Param.VAArray.push("Sarah_Victory_1");
			Yanfly.Param.VAArray.push("Sarah_Victory_2");
			Yanfly.Param.VAArray.push("Sarah_Victory_3");
		}
		if (actor._actorId === 22) {
			Yanfly.Param.VAArray.push("Rex_Victory_1");
			Yanfly.Param.VAArray.push("Rex_Victory_2");
			Yanfly.Param.VAArray.push("Rex_Victory_2");
		}
    }, this);
	if (!Yanfly.Param.VASkipper) {
		Yanfly.Param.VAPlayName = Yanfly.Param.VAArray[Math.floor(Math.random() * Yanfly.Param.VAArray.length)];
		AudioManager.playSe({name: Yanfly.Param.VAPlayName, pan: 0, pitch: 100, volume: 100});
		// Yanfly.Param.VASkipper = 1;
    }
};
